...
if st.button("Check"):
    ...
    if pred_label > 0:
        ...
        st.write(
            f"It's a Positive News. Confidence Level is {np.round(pred_prob, 3)}%  It's real news, you can trust it.")
    else:
        st.write(
            f"It's a Negative News. Confidence Level is {np.round(100 - pred_prob, 3)}%  Beware!! It's a Fake News.")
